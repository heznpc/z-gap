...
PRH attributes convergence to a shared statistical model of \emph{reality}~\cite{huh2024platonic}. We propose an alternative: convergence reflects the cognitive homogeneity of its sole training species.

\paragraph{Species-level constraint.}
Homo sapiens is among the most genetically homogeneous large mammals. A single group of 50 chimpanzees harbors more nucleotide diversity than all 8 billion humans~\cite{pradomartinez2013apes}. A severe bottleneck ${\sim}930{,}000$ years ago reduced the breeding population to ${\sim}1{,}280$ individuals, erasing an estimated 65.85\% of genetic diversity~\cite{hu2023bottleneck}. All LLM training data is produced by this genetically constrained species; the apparent universality of $Z$ may reflect the narrow bandwidth of human perception rather than a Platonic structure of the world.

\paragraph{WEIRD double constraint.}
Internet text further narrows this sample. Henrich et al.\ (2010)~\cite{henrich2010weird} showed that WEIRD (Western, Educated, Industrialized, Rich, Democratic) subjects---from whom the majority of training data originates---are ``among the least representative populations one could find for generalizing about humans.'' $Z$ is thus doubly constrained: by species-level cognitive architecture and by the cultural subsample that dominates $\Dtrain$.
...
\begin{itemize}[nosep]
  \item \textbf{Quorum sensing.} Autoinducer-2 (AI-2) is produced by $>$50\% of sequenced bacterial species, yet each species uses different receptors and triggers different gene cascades~\cite{waters2005quorum}. Same representation, species-specific interpretation.
...
A structural asymmetry deepens this gap. Code is \emph{modality-complete}: its text encodes the entirety of its meaning, with no reliance on intonation, gesture, or shared background. Natural language is \emph{modality-incomplete}: meaning depends on non-verbal channels that embedding models cannot access. High-context languages~\cite{hall1976beyond} amplify this incompleteness: Korean routinely omits subjects recoverable from social context, and mimetic expressions like \emph{salgeumsalgeum} (``stealthily'') encode embodied sensory meaning with no clean English mapping. Embedding models trained on text alone capture only the explicit channel---for code this is everything; for NL, especially high-context NL, it is a lossy projection. Recent work on World English varieties confirms this asymmetry empirically: AI-generated text maintains high semantic similarity while systematically erasing pragmatic and cultural markers, with pragmatic markers $1.9\times$ more vulnerable than lexical ones~\cite{culturalghosting2026}.
...
Judgment operations show \emph{higher} cross-lingual invariance than computational operations ($R_J = 3.79$ vs.\ $R_C = 1.91$ for MiniLM; $p=1.0$)---the opposite of our prediction. Computational operations have higher $d_{\text{intra}}$ (0.295 vs.\ 0.191, $p < 0.0001$), meaning they are \emph{less} invariant at the description level. A structural factor compounds this: NL is \emph{modality-incomplete}---full meaning depends on non-verbal context that text-only embeddings cannot capture---while code is modality-complete, explaining why NL-code alignment succeeds (\S\ref{sec:communicability}) where NL-NL cross-lingual invariance fails. Cross-dialectal analysis further reveals that this gap is \emph{continuous}: $R$ degrades progressively from within-dialect to cross-dialect to cross-lingual comparisons, confirming that communicability is not a binary property but a function of linguistic distance and context dependence.

\paragraph{Why P2 fails---and why the failure is informative.}
The P2 prediction was wrong. Before interpreting the failure, we acknowledge a prior question: \emph{was P2 the right test?} P2 measured cross-lingual invariance of NL \emph{descriptions} as a proxy for $\Zsem$ convergence. But if $\Zsem$ convergence occurs at the execution level while NL descriptions remain surface-level glosses, then embedding-based $R$ on descriptions may not reach $\Zsem$ at all---making P2 a test of the encoding interface, not of $Z$ itself. This possibility does not rescue the prediction; it questions its design. A stronger P2 would embed execution traces or I/O behaviors directly (cf.\ Limitations). With this caveat, three complementary hypotheses explain the observed dissociation.

\emph{(i) English-pivot alignment.}
Multilingual transformers process non-English input through intermediate English-like representations in middle layers~\cite{wendler2024llamas}. Judgment operations use abstract vocabulary (``evaluate,'' ``prioritize'') whose English glosses are near-identical across languages, yielding high $R_J$. Computational operations use domain-specific terminology (``transpose,'' ``jeongnyeol'') that maps to diverse English surface forms, lowering $R_C$. Under this account, P2 does not measure $\Zsem$ convergence at all---it measures English-vocabulary alignment in the embedding space, and its failure is a measurement artifact rather than a refutation of convergence.

\emph{(ii) Description vs.\ execution semantics.}
SemCoder~\cite{wan2024semcoder} distinguishes three levels of program understanding: high-level \emph{description} (what the function is called), local \emph{execution} (what each step does), and input-output \emph{behavior} (what the function computes). Our embedding-based $R$ metric operates at level~(1)---it compares NL descriptions, not execution traces. Descriptions may diverge across languages precisely because they are surface-level glosses of a shared computational denotation. The P2 failure is thus consistent with $\Zsem$ convergence at the execution level (levels 2--3), confirming that the gap lies in the NL-to-$Z$ encoding path, not in $Z$ itself.

\emph{(iii) $\Dtrain$ mediation.}
English-dominant training causes all languages to be processed partly through English-centric semantic hubs~\cite{wu2025semantichub}. The observed $R$ reflects not only $\Zsem$ but also a $\Dtrain$-dependent bias term: $\Zsem(\text{observed}) = \Zsem(\text{ideal}) + \mathrm{bias}(\Dtrain)$. Computational operations, whose domain-specific vocabulary is more sensitive to $\Dtrain$ composition, accumulate larger bias---explaining why $R_C < R_J$ despite the prediction that computational semantics should be more universal.

Together, these hypotheses localize the communicability gap to the NL-to-$Z$ encoding interface rather than to $Z$ itself. However, an alternative reading deserves acknowledgment: the P2 failure could indicate that the $\Zsem$/$\Zproc$/$\Zprag$ boundaries do not carve $Z$ at its joints---that the three-layer model is an over-specification, or that the layers interact in ways that prevent clean empirical separation at the description level. We cannot distinguish this structural critique from hypotheses (i)--(iii) with the current pilot; doing so requires execution-level probing (embedding code behavior rather than NL descriptions) and multi-scale model comparison (see \S\ref{sec:conclusion}).
...
$\Zsem$ is culture-invariant for purely computational domains; $Z$ carries cultural inflection for judgment-involving domains---requiring $Z(c)$ rather than $Z$. This inflection is amplified in high-context cultures~\cite{hall1976beyond}, where meaning relies on non-verbal channels---subject omission, social register, embodied reference---that text-only models cannot capture~\cite{park2024pragmatic}. The modality incompleteness of NL thus varies across cultures, making the communicability gap a function of both $\Dtrain$ and the source culture's context dependence. Furthermore, the gap is not categorical but \emph{continuous}: dialectal variants within a language occupy intermediate positions, as shown by dialect-aware embedding benchmarks~\cite{faisal2025swan} and evidence that LLMs treat dialects as stigmatized deviations rather than points on a continuum~\cite{bui2025dialect}. $\Dtrain$ dependence creates ethical risks: computational colonialism (English-derived norms as ``neutral'' $Z$), consent opacity, and cultural feedback loops ($Z$-projected outputs re-enter $\Dtrain$). Korean programmers face a double translation: thought (Korean) $\to$ code (English syntax) $\to$ execution; representation-level systems could eliminate both steps~\cite{li2026reasoning}. The biological precedents in \S\ref{sec:whydoes} suggest this boundary is not unique to AI: convergent signaling without communicability is the norm across biological systems, from bacterial quorum sensing to primate alarm calls. The NL-code communicability gap may be an instance of a more general principle: convergent representations arise readily under shared environmental pressure, but communicability requires additional alignment that convergence alone does not provide. See Appendix~\ref{app:ethics} for full analysis.
...
Huh et al.\ (ICML 2024)~\cite{huh2024platonic} established convergence; Ziyin et al.\ (2025)~\cite{ziyin2025proof} proved it for deep linear networks; Gr\"{o}ger et al.\ (2026)~\cite{groger2026aristotelian} showed global convergence is confounded but local topology persists. We contribute a biological/anthropological critique (\S\ref{sec:whydoes}): convergence may reflect species-level cognitive homogeneity---Homo sapiens' unusually low genetic diversity~\cite{pradomartinez2013apes,hu2023bottleneck}---rather than a Platonic structure of reality. Biology provides ample precedent for convergent signals without communicability~\cite{waters2005quorum,seyfarth1980vervet,magrath2015eavesdropping}.

\paragraph{Multilingual representations.}
The Semantic Hub Hypothesis~\cite{wu2025semantichub} demonstrates shared representations; middle-layer alignment is strongest across 1,000+ language pairs~\cite{middlelayer2025}. However, multilingual $\neq$ multicultural (Rystr{\o}m et al., 2025)~\cite{rystrom2025multicultural}: LLMs produce different moral judgments across languages, reflect creator ideology~\cite{buyl2025ideology}, and Sapir-Whorf effects are measurable in LLM outputs across 13 languages~\cite{ray2025sapirwhorf}.
...
Tokenizers introduce up to 15$\times$ cost unfairness across languages~\cite{petrov2024tokenizers}, with dialectal varieties suffering 10--15$\times$ tokenization premiums~\cite{tokenbiases2025}. Software is ``natural''---statistically predictable like NL~\cite{hindle2012naturalness}. Dialect-aware benchmarks~\cite{faisal2025swan} and studies of dialect bias in LLMs~\cite{bui2025dialect} reveal that the communicability gap extends within languages, not just across them.

\paragraph{Neuroscience and philosophy.}
Hyperalignment~\cite{haxby2011hyperalignment} and RSA~\cite{kriegeskorte2008rsa} show cross-brain representational convergence---the biological analogue of PRH---yet shared geometry does not resolve the private language problem~\cite{wittgenstein1953}. Mahowald et al.\ (2024)~\cite{mahowald2024dissociating} dissociate formal from functional linguistic competence in LLMs, paralleling our $\Zsem$/$\Zproc$ stratification. The modality gap in contrastive learning~\cite{liang2022modalitygap} persists even after alignment; closing it helps group-level but not instance-level tasks~\cite{grassucci2026modgap}, consistent with our prediction that $\Zsem$ converges while $\Zprag$ does not.
...
%% ============================================================
%% RESPONSIBLE NLP RESEARCH CHECKLIST (ARR requirement)
%% ============================================================

\section*{Responsible NLP Research Checklist}

This checklist follows the ARR Responsible NLP Research requirements.

\subsection*{A. Limitations}
See the Limitations section above. Key limitations: PRH for code is a conjecture, not a theorem; the pilot measures description-level rather than execution-level convergence; the $Z$ stratification is a conceptual framework with initial but not definitive evidence; and disambiguation conservation is a design heuristic, not a formal theorem.

\subsection*{B. Potential Risks}
See the Ethics Statement above. Key risks include cultural bias in learned representations ($\Dtrain$-dependent $Z$ encoding Anglo-American defaults as ``neutral''), consent opacity in representation-level systems, and cultural feedback loops through recursive training on $Z$-projected outputs.

\subsection*{C. Models and Data}

\paragraph{Models used in the pilot experiment.}
\begin{itemize}[nosep]
  \item \textbf{paraphrase-multilingual-MiniLM-L12-v2} (sentence-transformers): 118M parameters, multilingual sentence embedding model, Apache 2.0 license. Intended use: sentence similarity and semantic search. Used for P2, P7, and NL-code alignment experiments.
  \item \textbf{multilingual-e5-large-instruct} (Microsoft): 560M parameters, multilingual text embedding model, MIT license. Intended use: text retrieval and semantic similarity. Used for P2 and P3 probing experiments.
  \item \textbf{UniXcoder-base} (Microsoft): 125M parameters, unified cross-modal pre-trained model for code representation, MIT license. Intended use: code search, code generation, and code-NL alignment. Used for NL-code alignment experiments.
\end{itemize}

\paragraph{Data.}
All stimuli (100 operations $\times$ 5 languages = 500 base descriptions, plus dialectal variants totaling approximately 1,800 stimuli) were created by the authors. No existing datasets were used. No personally identifiable information is included. Stimuli are available in the experiment repository.

\subsection*{D. Computational Budget and Reproducibility}

\paragraph{Compute.}
All embedding experiments were run on a single NVIDIA A100 GPU (40GB). Total GPU time: approximately 4 hours for all embedding computations across all models and stimuli. Estimated electricity cost: $<$\$5. No large-scale training was performed; all models were used in inference mode only with pre-trained weights.

\paragraph{Hyperparameters.}
Models were used with default inference settings. For sentence-transformers (MiniLM, E5-large): default pooling (mean), maximum sequence length 512 tokens, cosine similarity metric. For UniXcoder: default settings with maximum sequence length 512 tokens. No hyperparameter tuning was performed, as all experiments use pre-trained models in a zero-shot evaluation setting.

\paragraph{Results reporting.}
All reported results (discriminability ratios $R$, probing accuracies, drift values) are from single runs. The embedding computation is deterministic (no sampling or temperature parameters), so re-running produces identical results. Statistical significance tests (permutation tests) use 10,000 permutations. Probing classifier results (P3) use 5-fold cross-validation with mean accuracy reported.

\subsection*{E. AI Assistant Disclosure}
An AI assistant (Claude, Anthropic) was used during manuscript preparation for writing assistance, literature survey support, and LaTeX formatting. All scientific claims, experimental design, analysis, and interpretations are the authors' own. The AI assistant did not design experiments, run analyses, or generate results.
