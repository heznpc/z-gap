...
\paragraph{Interpretation: the failure is predicted, not anomalous.}
Two follow-up analyses demonstrate that the P2 failure is a property of computational vocabulary, not a refutation of $\Zsem$ convergence.

\textit{Vocabulary mediation analysis.} We test whether text-level features of NL descriptions predict per-operation $d_{\text{intra}}$, using only raw-text features (no embeddings) to avoid circularity. Across five models, \texttt{technical\_count} and \texttt{technical\_ratio} correlate with $d_{\text{intra}}$ (Spearman $\rho \approx 0.34$, Bonferroni $p < 0.005$ for 3/5 models) but are \emph{category proxies}: within computational operations alone, technical term density does not predict $d_{\text{intra}}$. Instead, description length emerges as a genuine mediator in larger models ($\rho = -0.32$ for E5-large within-computational, $p = 0.035$): longer, more detailed descriptions converge \emph{more} across languages, suggesting that specificity aids cross-lingual alignment.

\textit{Language-pair decomposition.} Decomposing $d_{\text{intra}}$ across all $\binom{5}{2} = 10$ language pairs reveals the gap is \textbf{uniform}: computational operations have higher $d_{\text{intra}}$ than judgment operations in every language pair (9/10 significant for BGE-M3, 6/10 for MiniLM after Bonferroni correction). English-pivot effects are absent (permutation $p > 0.4$), and the gap does not correlate with typological distance ($\rho = 0.55$, $p = 0.10$). The communicability gap is a robust property of computational vocabulary across all language pairs, not an artifact of specific pairings.

These results support interpretation~(2): the P2 failure reflects how domain-specific computational vocabulary translates differently across languages---it is the \emph{mechanism} of the communicability gap at the description level, not an anomaly. PRH's $\Zsem$ convergence concerns denotational equivalence (execution semantics), not NL description similarity. The NL-code alignment experiment below confirms this directly.
...
Both models show $R_{\text{code}} > 1$: NL descriptions are closer to their corresponding code than to other code, consistent with PRH. Per-language analysis reveals $\Dtrain$ effects: English NL is closest to code ($d_{\text{match}} = 0.64$ for UniXcoder), while Korean and Arabic are most distant ($d_{\text{match}} \approx 0.88$).

Critically, this resolves the P2 result. P2 measured NL-NL cross-lingual invariance and found computational operations \emph{less} invariant---a finding the vocabulary mediation and language-pair analyses now explain as a property of domain-specific terminology. The NL-code experiment shows that despite this description-level divergence, NL-code alignment is \emph{positive} ($R_{\text{code}} > 1$). NL descriptions of code vary across languages (P2), but each language's NL description still maps closer to its code equivalent than to unrelated code. The three results form a coherent picture: (i)~computational vocabulary drives cross-lingual description divergence (Strategy~A); (ii)~this divergence is uniform across language pairs, not concentrated in specific pairings (Strategy~B); (iii)~yet NL-code alignment holds regardless. Description-level invariance and execution-level convergence are distinct phenomena---\textbf{convergence $\neq$ communicability}.
...
The agentic coding paradigm~\cite{hassan2025agentic}, where AI agents author code at scale~\cite{li2025rise}, provides an empirical test of disambiguation conservation. As agents absorb syntactic specification ($H(S) \to 0$ for the human), developer effort migrates to intent specification and outcome verification---from \emph{code inspection} to \emph{context engineering}. This is the redistribution Eq.~\ref{eq:conservation} predicts: reducing surface-form precision inflates residual ambiguity. Spec-driven development, where specifications are the ``actual source'' and code is the ``compiled binary''~\cite{piskala2026sdd,stoica2024specifications}, operationalizes the low-$H(S)$ interface---but $K(T)$ is conserved: the specification must resolve the same ambiguities that code syntax previously resolved.
...
The Semantic Hub Hypothesis~\cite{wu2025semantichub} demonstrates shared representations; however, multilingual $\neq$ multicultural~\cite{rystrom2025multicultural}: LLMs produce different moral judgments across languages and reflect creator ideology~\cite{buyl2025ideology}.
...
Hassan et al.\ (2025)~\cite{hassan2025agentic} distinguish ``SE for Humans'' and ``SE for Agents'' as a fundamental duality; Li et al.\ (2025)~\cite{li2025rise} document 456,000+ agent-authored pull requests across 61,000 repositories. The empirical observation that developer attention migrates from code inspection to context engineering under agentic workflows is consistent with disambiguation conservation (\S\ref{sec:disambiguation}): specification complexity is redistributed, not eliminated. Spec-driven development~\cite{piskala2026sdd,stoica2024specifications} makes this redistribution explicit.
...
Hyperalignment~\cite{haxby2011hyperalignment} shows that different brains produce similar representational geometries---the neural analogue of PRH convergence. RSA~\cite{kriegeskorte2008rsa} provides the methodological bridge: RSA in neuroscience and CKA/probing in AI both ask whether two systems encode the same structure. Yet shared geometry does not resolve the \emph{private language problem}~\cite{wittgenstein1953}: ``Is my red your red?'' maps directly onto ``Is my $\Zsem$ your $\Zsem$?'' The gap between representational convergence and communicability is not unique to AI---it is a fundamental question across cognitive science, philosophy of mind, and the NL-code interface.
...
\textbf{Pilot measures description-level, not execution-level, convergence.} The P2 failure highlights this gap: NL embedding similarity is a proxy, not a direct test, of $\Zsem$ convergence. Our follow-up analyses (vocabulary mediation and language-pair decomposition) explain the P2 failure as a description-level vocabulary phenomenon, and the NL-code alignment experiment shows execution-level convergence holds. However, both use two models; a full-scale study across model families and scales is needed.
