...
Three patterns emerge. First, \textbf{$\Dtrain$ modulates NL-code alignment}: English consistently shows the highest $R_{\text{code}}$ (1.21--1.28), while Korean and Arabic show the lowest (1.01--1.16), tracking language representation in code training corpora. Second, \textbf{the NL-only models in our set show higher $R_{\text{code}}$ than the code-trained one}: E5-large (1.20 aggregate) and BGE-M3 (1.16) exceed UniXcoder (1.06) and Nomic (1.07). We report this as a within-set observation, not a categorical claim: the code-trained category here is a single pure code-trained model (UniXcoder, the oldest and smallest in the set) plus one NL+code hybrid (Nomic); whether code-training categorically fails to improve NL-code alignment requires a broader modern code-trained set (e.g.\ CodeSage-Large-v2, Qodo-Embed), which we leave to future work (\S Limitations). Third, \textbf{the E5 family (same architecture, identical training recipe, varying dimension) shows partial scale-convergence}: aggregate $R_{\text{code}}$ rises $1.13$ (small, 384d) $\to 1.14$ (base, 768d) $\to 1.20$ (large, 1024d); the small-to-base jump is flat while base-to-large is steep, so P1's monotonic-with-scale prediction holds qualitatively but is non-linear in this regime.
...
\paragraph{Pretraining contamination caveat.} A stronger confound than surface lexical overlap is direct memorization. All 50 tier-1 computational operations are Python standard-library idioms (\texttt{sorted(lst)}, \texttt{max(lst)}, \texttt{len(lst)}, \texttt{collections.Counter(lst)}, ...), so the embedding models almost certainly observed exact or near-exact NL$\leftrightarrow$code co-occurrences during pretraining (docstrings, Stack Overflow, GitHub README files). The token-overlap controls above bound the \emph{surface lexical} component of the alignment but not the \emph{operation-level memorization} component. The OOD experiment in the next paragraph directly tests this confound; matched-perplexity controls remain future work.

\paragraph{Out-of-distribution NL-code alignment.}
We re-run the per-language $R_{\text{code}}$ matrix on 50 OOD operations: 30 tier-2 multi-step algorithms (binary search, merge sort, BFS, DFS, ...) and 20 tier-3 compositional algorithms (Bellman-Ford, topological sort, A$^{\!*}$, dynamic programming, ...). Mean NL description length is $180$ characters versus $55$ for tier 1; code is multi-line function bodies rather than 1-liners. The memorization hypothesis predicts $R_{\text{code}}$ should \emph{drop} on OOD; we observe the opposite for every model.
...
E5-base (NL)       & 1.64* & 1.23* & 1.22* & 1.19* & 1.36* & 1.31 & 1.14 \\
E5-large (NL)      & 1.61* & 1.28* & 1.25* & 1.22* & 1.37* & 1.33 & 1.20 \\
...
P3 is \textbf{supported in multilingual NL models but is model-class dependent}. The multilingual NL family (MiniLM, E5 small/base/large, BGE-M3) achieves $0.86$--$0.99$ category transfer and $0.86$--$0.98$ operation transfer---direct evidence that $\Zsem$ structure generalizes cross-lingually for models trained on multilingual NL. Code-trained (UniXcoder: $0.67$ / $0.18$) and mixed NL+code (Nomic v1.5: $0.62$ / $0.23$) models show much weaker cross-lingual operation transfer despite near-perfect English training accuracy, indicating their NL representations do not align cross-lingually even when within-English performance is adequate (both code-oriented models in our set; the broader code-trained comparison noted for the NL-code result applies here too). The E5 family alone exhibits a clean P3 scale-convergence echo of the NL-code alignment pattern: operation transfer rises $0.89$ (small, 384d) $\to 0.96$ (base, 768d) $\to 0.98$ (large, 1024d), under fixed architecture and training recipe. This refines the original P3 claim: cross-lingual $\Zsem$ separability is a property of the multilingual NL training distribution, not an intrinsic property of all embedding spaces with $R_{\text{code}} > 1$.

\paragraph{Within-variant, paraphrase, and cross-lingual distances (scope note).}
A pilot probe measured embedding distances at three levels: within-English orthographic variants (American vs.\ British / Indian spelling), within-language paraphrases, and cross-lingual pairs. We deliberately do \emph{not} draw a graded-continuum conclusion from it, for two reasons. First, the within-English ``dialect'' variants in this pilot are orthographically near-identical to the originals, so all models collapse them to $d \approx 0.01$--$0.03$; this measures orthographic insensitivity, not dialectal robustness. Second, the paraphrase-versus-cross-lingual ordering is \emph{not} stable across models: only E5-large shows $d_{\text{paraphrase}} < d_{\text{cross-lingual}}$, while MiniLM and BGE-M3 show the reverse. A defensible cross-dialect evaluation requires phonologically and lexically distinct varieties from native-validated corpora---e.g.\ Egyptian Arabic from MADAR~\cite{bouamor2018madar} or NADI~\cite{abdulmageed2024nadi}, and Gyeongsang Korean from the AI~Hub dialect corpus~\cite{aihub2024kodialect}---evaluated with established dialect-robustness methodology~\cite{ziems2022value} and multilingual embedding benchmarks~\cite{enevoldsen2025mmteb}. We identify this as the immediate next step rather than a present claim; the only robust statement our pilot supports is that embedding models are insensitive to within-English orthographic variation relative to lexical (paraphrase) and language-level variation.
...
\textbf{Pilot measures description-level, not execution-level, convergence.} The P2 failure highlights this gap: NL embedding similarity is a proxy, not a direct test, of $\Zsem$ convergence. Our follow-up analyses (vocabulary mediation and language-pair decomposition) explain the P2 failure as a description-level vocabulary phenomenon, and the NL-code alignment experiment confirms execution-level convergence across seven models and five languages (35/35 cells significant, plus a parallel 35/35 result on OOD multi-step / compositional stimuli). However, all seven models are encoder-style sentence embedders. Our claims are scoped to that representation class; decoder-only LLM hidden states may behave differently. Critically, frontier closed models (GPT, Claude, Gemini) expose no embedding or hidden-state API, so representation-level probing of the most capable models is infeasible by API design---the decoder-only extension is necessarily restricted to open-weight LLMs (e.g.\ Llama, Qwen).

\textbf{The $Z$ stratification is a conceptual framework.} While the pilot provides supporting evidence (P7 supported, P2 failure explained by vocabulary mediation, P3 supported on 7 models with model-class dependence---multilingual NL strong, code-trained / NL+code mixed weak), large-scale probing across open-weight decoder-only LLM families (e.g.\ Llama, Qwen hidden states) remains future work. The operation-level OOD stimuli (\texttt{tier2\_multistep.json}, \texttt{tier3\_compositional.json}) are analyzed in \S\ref{sec:pilot} (Strategy~F, 35/35 OOD cells), not deferred.
...
\textbf{Code-trained model coverage.} The ``NL-only $>$ code-trained'' observation (\S\ref{sec:pilot}) and the weaker P3 cross-lingual transfer of code-oriented models rest on a single pure code-trained model (UniXcoder) plus one NL+code hybrid (Nomic). A categorical claim about code-training requires a broader modern code-trained set; CodeSage-Large-v2 and Qodo-Embed are the intended additions, deferred here because CodeSage v2's distributed remote code is currently incompatible with recent \texttt{transformers} releases and loads only under a pinned legacy environment. We therefore treat the code-trained comparison as a scoped robustness extension, not a settled result.
...
\textbf{Pretraining contamination of NL-code stimuli (partially addressed).} The tier-1 50 computational operations are common stdlib idioms with high probability of appearing in every embedding model's pretraining corpus. The OOD experiment (\S\ref{sec:pilot}, "Out-of-distribution NL-code alignment") shows that $R_{\text{code}} > 1$ in fact \emph{strengthens} on tier-2 / tier-3 multi-step and compositional algorithms (aggregate $R_{\text{code}}$ rises across all 7 models), demonstrating that the alignment effect is not primarily memorization-driven. The remaining residual confound---matched-perplexity controls within the tier-1 set---is left to future work.
...
A Korean speaker maps all four spacing variants to the same meaning effortlessly. The ideal encoder should do the same---collapsing surface variation while preserving semantic distinction. Current tokenizers do not; character-level models (ByT5~\cite{xue2022byt5}, CANINE~\cite{clark2022canine}) and tokenizer-free byte-level architectures (Byte Latent Transformer~\cite{pagnoni2024blt}) partially address this but sacrifice other capabilities.
