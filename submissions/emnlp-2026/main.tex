...
\paragraph{Compute.}
All embedding experiments were run on macOS with Apple Silicon (M-series), using CPU inference via the \texttt{sentence-transformers} library. No GPU was used. Total compute time: approximately 4 hours for all embedding computations across all models and stimuli. Estimated electricity cost: negligible (consumer laptop). No large-scale training was performed; all models were used in inference mode only with pre-trained weights.
